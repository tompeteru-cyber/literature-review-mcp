...
export class DatabaseManager {
   ...
   */
  async storePaper(paper: Paper): Promise<void> {
   ...
   */
  async storeSearchResults(results: any): Promise<void> {
    if (this.useSupabase && this.supabaseClient) {
      await this.supabaseClient
        .from('search_results')
        .insert({
   ...
   */
  async storeMethodology(paperId: string, methodology: any): Promise<void> {
    if (this.useSupabase && this.supabaseClient) {
      await this.supabaseClient
        .from('methodologies')
        .insert({
   ...
   * Store research gaps
   */
  async storeResearchGaps(gaps: any[]): Promise<void> {
   ...
   */
  async storeSynthesis(synthesis: any): Promise<void> {
    if (this.useSupabase && this.supabaseClient) {
      await this.supabaseClient
   ...
   * Store AHP results
   */
  async storeAHPResults(results: any[]): Promise<void> {
   ...
   */
  async storeChapter(chapterNumber: string, title: string, content: string, format: string, metadata: any): Promise<void> {
    if (this.useSupabase && this.supabaseClient) {
      await this.supabaseClient
        .from('chapters')
        .insert({
   ...
   */
  async getAnalyses(analysisType?: string, limit: number = 10): Promise<StoredAnalysis[]> {
    let analyses: any[] = [];
   ...
   */
  async getResearchGaps(gapType?: string): Promise<any[]> {
    let gaps: any[] = [];
   ...
   */
  async getSynthesisResults(limit: number = 5): Promise<any[]> {
    let results: any[] = [];
    ...
    if (this.useSupabase && this.supabaseClient) {
      ...
      results = await this.sqliteDb.all(
        'SELECT * FROM synthesis_results ORDER BY created_at DESC LIMIT ?',
        [limit]
      );
    }
   ...
   */
  async getAHPResults(limit: number = 100): Promise<any[]> {
    let results: any[] = [];
    ...
    if (this.useSupabase && this.supabaseClient) {
      ...
      results = await this.sqliteDb.all(
        'SELECT * FROM ahp_results ORDER BY rank ASC LIMIT ?',
        [limit]
      );
    }
  ...
  async getChapters(): Promise<any[]> {
    ...
    if (this.useSupabase && this.supabaseClient) {
      ...
      chapters = await this.sqliteDb.all(
