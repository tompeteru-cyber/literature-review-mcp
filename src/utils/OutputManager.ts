...
export class OutputManager {
   ...
   */
  async savePresentation(presentationData: any, format: 'powerpoint' | 'reveal.js' | 'beamer' | 'google-slides' = 'powerpoint'): Promise<string> {
   ...
   */
  async savePaperSummary(summaryData: any, theme: string, format: 'markdown' | 'powerpoint' | 'html' | 'latex' = 'markdown'): Promise<string> {
...
`;
  ...
  private getPresentationExtension(format: string): string {
    switch (format) {
      case 'reveal.js':
        return 'html';
      case 'beamer':
  ...
  private formatAsPresentationMarkdown(data: any): string {
...
**Visual Elements:**
${slide.visualElements.map((element: string) => `- ${element}`).join('\n')}
` : ''}
...
`;
  ...
  private formatAsRevealJS(data: any): string {
    const slides = Array.isArray(data.slides) ? data.slides : [];
...
<body>
        ...
        <div class="slides">
            ...
            <section>
                ...
                <div class="visual-elements">
                    ${slide.visualElements.map((element: string) => `<div class="visual-element">${element}</div>`).join('')}
                </div>` : ''}
  ...
  private formatAsBeamer(data: any): string {
    const slides = Array.isArray(data.slides) ? data.slides : [];
...
${slide.visualElements && slide.visualElements.length > 0 ? `
\\begin{itemize}
${slide.visualElements.map((element: string) => `\\item ${element}`).join('\n')}
\\end{itemize}` : ''}
\\end{frame}
`).join('')}
...
\\end{document}`;
  ...
  private formatPaperSummaryAsMarkdown(data: any, theme: string): string {
...
${Array.isArray(evidence.keyPoints) ? evidence.keyPoints.map((point: string, i: number) =>
...
${Array.isArray(limitations) ? limitations.map((limitation: string) => `- ${limitation}`).join('\n') : 'No limitations documented'}
...
`;
  ...
  private formatPaperSummaryAsHTML(data: any, theme: string): string {
    const summary = data.summary || data;
    const evidence = summary.evidence || {};
    const methods = summary.toolsAndMethods || {};
    const limitations = summary.limitations || [];
...
<body>
    ...
    <div class="section">
        ...
        ${Array.isArray(evidence.keyPoints) ? evidence.keyPoints.map((point: string) =>
          `<div class="evidence-item">${point}</div>`).join('') : '<p>No key points available</p>'}
    </div>
    ...
    <div class="section">
        <h2>Limitations</h2>
        ${Array.isArray(limitations) ? limitations.map((limitation: string) =>
          `<div class="limitation">${limitation}</div>`).join('') : '<p>No limitations documented</p>'}
    </div>

    <div class="section">
        <h2>Preliminary Findings</h2>
        ${Array.isArray(findings.findings) ? findings.findings.map((finding: string, i: number) =>
          `<div class="finding-item"><strong>${i + 1}.</strong> ${finding}</div>`).join('') : '<p>No findings available</p>'}
    </div>
  ...
  private formatPaperSummaryAsLatex(data: any, theme: string): string {
    const summary = data.summary || data;
    const evidence = summary.evidence || {};
    const methods = summary.toolsAndMethods || {};
    const limitations = summary.limitations || [];
...
\\begin{itemize}
${Array.isArray(limitations) ? limitations.map((limitation: string) =>
  `  \\item ${limitation.replace(/[_&%$#]/g, '\\$&')}`).join('\n') : '  \\item No limitations documented'}
\\end{itemize}
\\end{tcolorbox}

\\subsection{Preliminary Findings}
\\begin{enumerate}
${Array.isArray(findings.findings) ? findings.findings.map((finding: string) =>
  `  \\item ${finding.replace(/[_&%$#]/g, '\\$&')}`).join('\n') : '  \\item No findings available'}
\\end{enumerate}
