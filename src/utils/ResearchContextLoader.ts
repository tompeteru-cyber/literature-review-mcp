...
export interface ResearchContext {
  ...
  literatureDistribution: {
    totalPapers: number;
    blockOutfitting: { count: number; percentage: number };
    postLaunchOutfitting: { count: number; percentage: number };
    majorGap: string;
  };

  identifiedGaps: {
    theoretical: { title: string; description: string };
    methodological: { title: string; description: string; limitation?: string };
    practical: { title: string; description: string };
  };

  researchDirections: {
    option1: { title: string; focus: string; rationale: string[] };
    option2: { title: string; focus: string; rationale: string[] };
  };
...
export class ResearchContextLoader {
   ...
   */
  async loadContext(forceReload: boolean = false): Promise<ResearchContext> {
    const now = Date.now();
   ...
   */
  private parseMarkdown(content: string): ResearchContext {
    const context: ResearchContext = this.getDefaultContext();
   ...
   */
  private parseOutfittingSection(content: string, sectionName: string): {
    description: string;
    environment: string;
    equipment: string;
    challenges: string[];
   ...
   */
  private parseGaps(content: string): ResearchContext['identifiedGaps'] {
    const gaps: ResearchContext['identifiedGaps'] = {
   ...
   * Parse research directions
   */
  private parseResearchDirections(content: string): ResearchContext['researchDirections'] {
    const directions: ResearchContext['researchDirections'] = {
   ...
   * Parse analysis guidelines
   */
  private parseAnalysisGuidelines(content: string): ResearchContext['analysisGuidelines'] {
    const guidelines: ResearchContext['analysisGuidelines'] = {
      alwaysIdentify: [],
   ...
   */
  private parseThemes(content: string): ResearchContext['themes'] {
    const themes: ResearchContext['themes'] = [];
   ...
   */
  private parsePapersAnalyzed(content: string): ResearchContext['papersAnalyzed'] {
    const result = {
      total: 0,
      blockOutfitting: 0,
      postLaunch: 0,
   ...
   */
  private getDefaultContext(): ResearchContext {
    return {
   ...
   */
  async getFilteredContext(contextType: string = 'full'): Promise<Partial<ResearchContext>> {
...
let loaderInstance: ResearchContextLoader | null = null;

export function getResearchContextLoader(contextFilePath?: string): ResearchContextLoader {
  if (!loaderInstance) {
